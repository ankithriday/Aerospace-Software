function varargout = gui_pend(varargin)
% GUI_PEND MATLAB code for gui_pend.fig
%      GUI_PEND, by itself, creates a new GUI_PEND or raises the existing
%      singleton*.
%
%      H = GUI_PEND returns the handle to a new GUI_PEND or the handle to
%      the existing singleton*.
%
%      GUI_PEND('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_PEND.M with the given input arguments.
%
%      GUI_PEND('Property','Value',...) creates a new GUI_PEND or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before gui_pend_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to gui_pend_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help gui_pend

% Last Modified by GUIDE v2.5 21-Mar-2018 21:18:08

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @gui_pend_OpeningFcn, ...
                   'gui_OutputFcn',  @gui_pend_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before gui_pend is made visible.
function gui_pend_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to gui_pend (see VARARGIN)

% Choose default command line output for gui_pend
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);
 set(handles.figure1,'Units','Pixels','Position',get(0,'ScreenSize'));
% UIWAIT makes gui_pend wait for user response (see UIRESUME)
% uiwait(handles.figure1);
set(handles.pt1vt,'Enable','off');
set(handles.pt1vt,'BackgroundColor','black'); 
set(handles.pt2vt,'Enable','off');
set(handles.pt2vt,'BackgroundColor','black'); 
set(handles.pzvx,'Enable','off');
set(handles.pzvx,'BackgroundColor','black');

% --- Outputs from this function are returned to the command line.
function varargout = gui_pend_OutputFcn(hObject, eventdata, handles) 
...
theta_dot1 = get(handles.td1,'string');
...
theta_dot2 = get(handles.td2,'string');
...
theta_dot1 = str2num(get(handles.td1,'string')); %thetad1
theta_dot2 = str2num(get(handles.td2,'string')); %thetad2
...
tspan = str2num(get(handles.timespan,'string')); %timespa
...
m1 = str2num(get(handles.m1,'string')); %m1
m2 = str2num(get(handles.m2,'string')); %m2
tspan = str2num(get(handles.timespan,'string')); %timespan
t = getappdata(0,'t');
y = getappdata(0,'y');
x1 = l1*sin(y(:,1));
z1 = -l1*cos(y(:,1));
